...
docker compose -f docker-compose-test.yml build friend-backend-test

print_info "Starting backend..."
docker compose -f docker-compose-test.yml up -d friend-backend-test
...
for i in {1..40}; do
    ...
    if [ $i -eq 40 ]; then
        ...
        docker compose -f docker-compose-test.yml logs friend-backend-test
        exit 1
    fi
    sleep 3
done
